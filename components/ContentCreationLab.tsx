      ...
      <div className="content-lab-window">
        ...
        <div className="content-lab-scroll" aria-live="polite">
          ...
          {runState === "complete" && <>
            ...
            {template.id === "article" && <article className="content-article-result"><header><span>内容稿 · 可继续编辑</span><small>约 650 字 · 2 处待核验</small></header><div className="content-article-layout"><aside><strong>内容大纲</strong><ol><li>为什么先从小范围开始</li><li>建立一个固定入口</li><li>每天只处理一种资料</li><li>如何保持习惯</li></ol></aside><section><span className="content-result-tag">生活方式 · 入门</span><h4>整理数字资料，不必从清空一切开始</h4><p>整理数字资料时，可以先为新增资料确定一个固定去处，再逐步处理历史文件。这样每天只需要完成一个容易执行的小动作。</p><p>第一周只需要选择一个入口，例如下载文件夹或手机相册。每天花十分钟处理当天新增的内容：留下需要的，删除确认无用的，再把其余内容放进名称清楚的文件夹。</p><blockquote><strong>待核验：</strong>如果需要加入效率提升数据，应补充可靠来源，不能直接生成百分比。</blockquote></section></div><footer>发布摘要：先建立可以坚持的小流程，再逐步整理历史资料。</footer></article>}
