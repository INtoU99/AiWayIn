...
export default async function ToolPage({ params }: ToolPageProps) {
  ...
  return (
    <main className="site-shell detail-shell tool-detail-page" id="top">
      ...
      <section className={`tool-detail-hero ${tool.tone}`}>
        ...
        <div className="detail-actions">
          ...
          {directDownloads.length > 0 && <div className="direct-download-panel"><strong>本站一键下载</strong><div>{directDownloads.map((download) => <a href={`/download/${download.toolId}/${download.platformId}`} key={download.platformId}>{download.label}<span aria-hidden="true">↓</span></a>)}</div></div>}
      ...
      <section className="detail-grid two-column">
        <article className="detail-panel detail-note-card">
        ...
        <article className="detail-panel soft-panel detail-note-card">
      ...
      <section className="detail-section">
        ...
        <ol className="installation-steps">{tool.installSteps.map((step, index) => <li className="detail-note-card" key={step}><span>{index + 1}</span><p>{step}</p></li>)}</ol>
      </section>

      <section className="success-panel">
        <div><span className="section-kicker">完成检查</span><h2>看到这些，说明已经可以使用</h2></div>
        <div className="success-list">{tool.successChecks.map((item) => <span className="detail-note-card" key={item}><b aria-hidden="true">✓</b>{item}</span>)}</div>
      </section>
      ...
      {relatedProjects.length > 0 && <section className="detail-section related-projects-section">
        ...
        <div className="related-project-grid">{relatedProjects.map((project) => <article className="related-project-card detail-note-card" key={project.id}>
      ...
      <section className="detail-section safety-detail">
        ...
        <div className="safety-grid">{tool.safetyNotes.map((item, index) => <article className="detail-note-card" key={item}><span>0{index + 1}</span><p>{item}</p></article>)}</div>
      </section>
