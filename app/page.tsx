...
const starterPaths = [
  { label: "从这里开始", title: "帮我判断怎么开始", description: "按设备和目标选择路线，不需要先理解专业术语。", action: "1 分钟找到路线", guideMode: "route" as const, tone: "blue" },
  { label: "安装之前", title: "查看设备与环境要求", description: "确认系统、Node.js 和必要的软件环境。", action: "开始环境自检", guideMode: "environment" as const, tone: "mint" },
];
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const observer = new IntersectionObserver(
      ([entry]) => setActiveHomeSection(entry.isIntersecting ? "guides" : "top"),
      { rootMargin: "-120px 0px -58% 0px", threshold: 0 },
    );
    observer.observe(guides);
  ...
  return (
    <main className="site-shell home-page">
      <span className="header-scroll-sentinel" ref={headerSentinelRef} aria-hidden="true" />
      <header className={`topbar${headerScrolled ? " is-scrolled" : ""}`} aria-label="网站顶部导航" ref={headerRef}>
        ...
        {menuOpen && <nav className="mobile-nav" id="mobile-navigation" aria-label="移动端导航">{[["首页", "#top"], ["安装指南", "#guides"], ["工具对比", "/compare"], ["工具导航", "/tools"], ["资源导航", "/resources"], ["常见问题", "/questions"], ["进阶计划", "/advanced"]].map(([label, href]) => href.startsWith("/") ? <BrowserNavigationLink href={href} key={href} onNavigate={() => setMenuOpen(false)}>{label}<span aria-hidden="true">→</span></BrowserNavigationLink> : <a className={href === `#${activeHomeSection}` ? "active" : undefined} href={href} key={href} aria-current={href === `#${activeHomeSection}` ? "page" : undefined} onClick={() => { setActiveHomeSection(href === "#guides" ? "guides" : "top"); setMenuOpen(false); }}>{label}<span aria-hidden="true">→</span></a>)}</nav>}
      </header>
      ...
      <section className="home-hero" id="top">
        <div className="home-hero-grid">
          ...
          <div className="starter-panel" role="group" aria-label="新手入口">
            {starterPaths.map((item) => <a className={`starter-card ${item.tone}`} href="#guides" key={item.title} onClick={() => setGuideMode(item.guideMode)}><span className="starter-label">{item.label}</span><span className="starter-content"><strong>{item.title}</strong><small>{item.description}</small><span className="starter-action">{item.action}</span></span><span className="arrow" aria-hidden="true">↗</span></a>)}
            <BrowserNavigationLink className="starter-card advanced-entry" href="/advanced"><span className="starter-label">工具已就绪</span><span className="starter-content"><strong>工具已经安装好了？</strong><small>从一个真实的小目标开始，尝试内容创作、应用构建、办公研究或 Agent 自动化。</small><span className="starter-action">试试进阶路线</span></span><span className="arrow" aria-hidden="true">↗</span></BrowserNavigationLink>
          </div>
        </div>
        <div className="home-tool-rail" aria-label="常用官方工具入口">
          <div className="home-tool-rail-heading"><strong>常用官方入口</strong><small>从可靠来源了解与下载工具</small></div>
          <div className="home-tool-rail-list">
            {catalogTools.filter((tool) => tool.featured).map((tool) => <BrowserNavigationLink href={`/tools/${tool.id}`} key={tool.id}><ToolLogo src={tool.logo} alt={tool.logoAlt} /><span>{tool.shortName}</span></BrowserNavigationLink>)}
          </div>
          <BrowserNavigationLink className="home-tool-rail-all" href="/tools">全部工具 <span aria-hidden="true">→</span></BrowserNavigationLink>
        </div>
      </section>
      ...
      <section className="section-block" id="guides">
        <div className="home-section-heading"><span className="section-kicker">安装指南</span><h2>从选择路线到确认环境</h2><p>先找到合适的工具，再确认设备和必要环境是否已经准备好。</p></div>
